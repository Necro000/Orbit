...
export function FilterChips({ filters, onFilterChange }: FilterChipsProps) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-3">
      ...
      <div className="flex items-center gap-1.5 p-1 bg-[#1A1A22] border border-[#2A2A35] rounded-[8px]">
          ...
          return (
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-[6px] text-xs font-medium transition-all ${
                isSelected
                  ? 'bg-[#6366F1] text-white shadow-sm shadow-[#6366F133]'
                  : 'bg-transparent text-[#8B8B96] hover:text-[#F5F5F7] hover:bg-[#22222C]'
              }`}
            >
              {IconComponent && (
                <IconComponent className={`w-3.5 h-3.5 ${isSelected ? 'text-white' : 'text-[#8B8B96]'}`} />
              )}
              <span>{label}</span>
            </button>
          );
      ...
      {/* Divider */}
      <div className="hidden sm:block w-px h-5 bg-[#2A2A35]" />
      ...
      {hasActiveFilter && (
        <button
          ...
          className="text-xs text-[#8B8B96] hover:text-[#EF4444] px-2 py-1 transition-colors flex items-center gap-1"
